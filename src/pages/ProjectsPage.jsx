...
export default function ProjectsPage() {
  ...
  const { toggleCompound } = useListingFav()
  const { openLogin } = useAuthModal()
  const [list, setList] = useState(null)
  const shown = list ?? projects
  ...
  return (
    <div className="wrap">
      <p className="crumb">Home &gt; Projects</p>
      <SearchBar compact />
      <div className="listing-layout" style={{ marginTop: 24 }}>
        <Filters />
        <div>
          <div className="listing-title">
            <h2>Recently Featured Projects</h2>
          ...
          <QueryState
            ...
            error={error}
            empty={!slice.length}
            emptyText="No projects match this view."
          >
            {slice.map((item) => (
              <PropertyRow
                key={item.id}
                ...
                to={`/projects/${item.id}`}
                onFav={(project) =>
                  toggleCompound(project, (id, favourite) => setList(markFavourite(shown, id, favourite)))
                }
              />
            ))}
